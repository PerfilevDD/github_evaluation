...
from .test_main import CheckResult, NamedCheck, all_checks

lookback_days = 90 # amount of days to check

def is_active(c: Tester) -> CheckResult:
    return multi_check(
        periodic_commits,
        periodic_releases
    )(c)
...
def periodic_commits(c: Tester) -> CheckResult:
    try:
        ...
        return CheckResult(pass_=total_commits >= 2, confidence=confidence)
    
    except Exception as e:
        return CheckResult(pass_=False, confidence=7, error=str(e))


def periodic_releases(c: Tester) -> CheckResult:
    try:
        ...
        return CheckResult(pass_=total_releases > 0, confidence=confidence)
    
    except Exception as e:
        return CheckResult(pass_=False, confidence=10, error=str(e))

def multi_check(*checks):
    def run_checks(c: Tester) -> CheckResult:
        for check in checks:
            result = check(c)
            if not result.pass_:
                return result
        return CheckResult(pass_=True, confidence=min(result.confidence for result in checks))
    return run_checks
